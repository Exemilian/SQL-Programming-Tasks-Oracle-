...
FROM EMPLOYEES JOIN SQ1 USING (DEPARTMENT_ID)
GROUP BY DEPARTMENT_ID)
SELECT EMPLOYEE_ID, LAST_NAME, JOB_ID,
DEPARTMENT_ID, SALARY, TRUNC(ASAL)
FROM EMPLOYEES JOIN JOBS USING (JOB_ID)
JOIN SQ1 USING (DEPARTMENT_ID) JOIN SQ2
USING (DEPARTMENT_ID)
WHERE (DEPARTMENT_ID, ABS(SALARY - ASAL)) IN
(SELECT DEPARTMENT_ID, MINSAL FROM SQ2)
...
SELECT LISTAGG(TABLE_NAME,',') WITHIN GROUP(ORDER BY TABLE_NAME ) TABLES
FROM TABLES
