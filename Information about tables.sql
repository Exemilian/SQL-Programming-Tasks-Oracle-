...
SELECT DISTINCT ALL_T.TABLE_NAME, NVL(PK.COLS,'-') "PK Columns",
       NVL(UK.COLS,'-') "UK Columns", 
       LISTAGG(NVL2(FK.COLS, FK.TABLE_NAME || '(' || FK.COLS || ')', '-'),', ') 
       WITHIN GROUP (ORDER BY FK.TABLE_NAME) OVER (PARTITION BY ALL_T.TABLE_NAME, UK.COLS) "FK Table Columns"
